...
interface SearchResult {
  ...
  nextStep: string | null;  // Added nextStep field
  documents: {
    type: string;
    fileUrl: string;
    fileKey: string;
...
export class SearchAllStatusComponent implements OnInit {
  ...
  formatNextStep(nextStep: string | null): string {
    if (!nextStep) return 'Not Available';
    if (nextStep === 'completed') return 'Completed';
