...
const fetchOnboarding = createAsyncThunk(
  ...
  async() => {
    ...
    const response = await axios.get('http://localhost:3000/api/employee/onboarding')
...
const updateOnboarding = createAsyncThunk(
  ...
  async(updates: Partial<Onboarding>) => {
    //validate the JWT token
    const response = await axios.put('http://localhost:3000/api/employee/onboarding/update', { updates })
    return response.data
  }
)
